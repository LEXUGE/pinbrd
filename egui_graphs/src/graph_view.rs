    ...
    L: Layout<S>,
{
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let mut meta = Metadata::load(ui, self.id);
    ...
    L: Layout<S>,
{
    ...
    pub fn new(g: &'a mut Graph<N, E, Ty, Ix, Dn, De>, id: egui::Id) -> Self {
        Self {
            g,
    ...
    pub fn clear_cache(ui: &mut Ui, id: egui::Id) {
        GraphView::<N, E, Ty, Ix, Dn, De, S, L>::reset_metadata(id, ui);
        GraphView::<N, E, Ty, Ix, Dn, De, S, L>::reset_layout(id, ui);
    }

    /// Resets navigation metadata
    pub fn reset_metadata(id: egui::Id, ui: &mut Ui) {
        Metadata::default().save(ui, id);
    }

    /// Resets layout state
    pub fn reset_layout(id: egui::Id, ui: &mut Ui) {
        ui.data_mut(|data| {
            data.insert_persisted(id.with(KEY_LAYOUT), S::default());
        });
    }
    ...
    fn sync_layout(&mut self, ui: &mut Ui) {
        ui.data_mut(|data| {
            let state = data
                .get_persisted::<S>(self.id.with(KEY_LAYOUT))
                .unwrap_or_default();
            let mut layout = L::from_state(state);
            layout.next(self.g);

            data.insert_persisted(self.id.with(KEY_LAYOUT), layout.state());
        });
    }
    ...
    fn sync_state(&mut self, meta: &mut Metadata) {
        ...
        // If graph is empty, we should set Bound rect to 0.
    ...
    fn handle_zoom(&self, ui: &Ui, resp: &Response, meta: &mut Metadata) {
        // If we are not hovered over, don't zoom!
        if !self.settings_navigation.zoom_and_pan_enabled || !resp.hovered() {
            return;
        }
